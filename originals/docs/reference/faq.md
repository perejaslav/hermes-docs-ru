...
### What LLM providers work with Hermes?[​](<#what-llm-providers-work-with-hermes> "Direct link to What LLM providers work with Hermes?")
...
### Does it work on Windows?[​](<#does-it-work-on-windows> "Direct link to Does it work on Windows?")
...
[/code]
### I run Hermes in WSL2. What's the best way to control my normal Windows Chrome?[​](<#i-run-hermes-in-wsl2-whats-the-best-way-to-control-my-normal-windows-chrome> "Direct link to I run Hermes in WSL2. What's the best way to control my normal Windows Chrome?")
...
### Does it work on Android / Termux?[​](<#does-it-work-on-android--termux> "Direct link to Does it work on Android / Termux?")
...
### Is my data sent anywhere?[​](<#is-my-data-sent-anywhere> "Direct link to Is my data sent anywhere?")
...
### Can I use it offline / with local models?[​](<#can-i-use-it-offline--with-local-models> "Direct link to Can I use it offline / with local models?")
...
### Can multiple people use one instance?[​](<#can-multiple-people-use-one-instance> "Direct link to Can multiple people use one instance?")
...
### What's the difference between memory and skills?[​](<#whats-the-difference-between-memory-and-skills> "Direct link to What's the difference between memory and skills?")
...
### Can I use it in my own Python project?[​](<#can-i-use-it-in-my-own-python-project> "Direct link to Can I use it in my own Python project?")
...
#### `hermes: command not found` after installation[​](<#hermes-command-not-found-after-installation> "Direct link to hermes-command-not-found-after-installation")
...
#### Python version too old[​](<#python-version-too-old> "Direct link to Python version too old")
...
#### Terminal commands say `node: command not found` (or `nvm`, `pyenv`, `asdf`, …)[​](<#terminal-commands-say-node-command-not-found-or-nvm-pyenv-asdf-> "Direct link to terminal-commands-say-node-command-not-found-or-nvm-pyenv-asdf-")
...
[/code]
#### Permission denied errors during install[​](<#permission-denied-errors-during-install> "Direct link to Permission denied errors during install")
...
[/code]
* * *
### Provider & Model Issues[​](<#provider--model-issues> "Direct link to Provider & Model Issues")
#### `/model` only shows one provider / can't switch providers[​](<#model-only-shows-one-provider--cant-switch-providers> "Direct link to model-only-shows-one-provider--cant-switch-providers")
...
#### Model not available / model not found[​](<#model-not-available--model-not-found> "Direct link to Model not available / model not found")
...
[/code]
#### Rate limiting (429 errors)[​](<#rate-limiting-429-errors> "Direct link to Rate limiting \(429 errors\)")
...
#### Context length exceeded[​](<#context-length-exceeded> "Direct link to Context length exceeded")
...
* * *
### Terminal Issues[​](<#terminal-issues> "Direct link to Terminal Issues")
#### Command blocked as dangerous[​](<#command-blocked-as-dangerous> "Direct link to Command blocked as dangerous")
...
#### `sudo` not working via messaging gateway[​](<#sudo-not-working-via-messaging-gateway> "Direct link to sudo-not-working-via-messaging-gateway")
...
#### Docker backend not connecting[​](<#docker-backend-not-connecting> "Direct link to Docker backend not connecting")
...
[/code]
* * *
### Messaging Issues[​](<#messaging-issues> "Direct link to Messaging Issues")
#### Bot not responding to messages[​](<#bot-not-responding-to-messages> "Direct link to Bot not responding to messages")
...
[/code]
#### Messages not delivering[​](<#messages-not-delivering> "Direct link to Messages not delivering")
...
#### Allowlist confusion — who can talk to the bot?[​](<#allowlist-confusion--who-can-talk-to-the-bot> "Direct link to Allowlist confusion — who can talk to the bot?")
...
#### Gateway won't start[​](<#gateway-wont-start> "Direct link to Gateway won't start")
...
[/code]
#### WSL: Gateway keeps disconnecting or `hermes gateway start` fails[​](<#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails> "Direct link to wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails")
...
#### macOS: Node.js / ffmpeg / other tools not found by gateway[​](<#macos-nodejs--ffmpeg--other-tools-not-found-by-gateway> "Direct link to macOS: Node.js / ffmpeg / other tools not found by gateway")
...
#### High token usage[​](<#high-token-usage> "Direct link to High token usage")
...
#### Session getting too long[​](<#session-getting-too-long> "Direct link to Session getting too long")
...
[/code]
* * *
### MCP Issues[​](<#mcp-issues> "Direct link to MCP Issues")
...
[/code]
#### Tools not showing up from MCP server[​](<#tools-not-showing-up-from-mcp-server> "Direct link to Tools not showing up from MCP server")
...
#### MCP timeout errors[​](<#mcp-timeout-errors> "Direct link to MCP timeout errors")
...
### How do profiles differ from just setting HERMES_HOME?[​](<#how-do-profiles-differ-from-just-setting-hermes_home> "Direct link to How do profiles differ from just setting HERMES_HOME?")
Profiles are a managed layer on top of `HERMES_HOME`. You _could_ manually set `HERMES_HOME=/some/path` before every command, but profiles handle all the plumbing for you: creating the directory structure, generating shell aliases (`hermes-work`), tracking the active profile in `~/.hermes/active_profile`, and syncing skill updates across all profiles automatically. They also integrate with tab completion so you don't have to remember paths.
### Can two profiles share the same bot token?[​](<#can-two-profiles-share-the-same-bot-token> "Direct link to Can two profiles share the same bot token?")
...
### Do profiles share memory or sessions?[​](<#do-profiles-share-memory-or-sessions> "Direct link to Do profiles share memory or sessions?")
...
### What happens when I run `hermes update`?[​](<#what-happens-when-i-run-hermes-update> "Direct link to what-happens-when-i-run-hermes-update")
...
### How many profiles can I run?[​](<#how-many-profiles-can-i-run> "Direct link to How many profiles can I run?")
...
* * *
## Workflows & Patterns[​](<#workflows--patterns> "Direct link to Workflows & Patterns")
### Using different models for different tasks (multi-model workflows)[​](<#using-different-models-for-different-tasks-multi-model-workflows> "Direct link to Using different models for different tasks \(multi-model workflows\)")
...
You can also be explicit in your prompt: _"Delegate a task to write social media posts about our product launch. Use your subagent for the actual writing."_ The agent will use `delegate_task`, which automatically picks up the delegation config.
...
### Running multiple agents on one WhatsApp number (per-chat binding)[​](<#running-multiple-agents-on-one-whatsapp-number-per-chat-binding> "Direct link to Running multiple agents on one WhatsApp number \(per-chat binding\)")
...
### Controlling what shows up in Telegram (hiding logs and reasoning)[​](<#controlling-what-shows-up-in-telegram-hiding-logs-and-reasoning> "Direct link to Controlling what shows up in Telegram \(hiding logs and reasoning\)")
...
[/code]
### Managing skills on Telegram (slash command limit)[​](<#managing-skills-on-telegram-slash-command-limit> "Direct link to Managing skills on Telegram \(slash command limit\)")
...
### Shared thread sessions (multiple users, one conversation)[​](<#shared-thread-sessions-multiple-users-one-conversation> "Direct link to Shared thread sessions \(multiple users, one conversation\)")
...
### Exporting Hermes to another machine[​](<#exporting-hermes-to-another-machine> "Direct link to Exporting Hermes to another machine")
...
### Moving a single profile to another machine[​](<#moving-a-single-profile-to-another-machine> "Direct link to Moving a single profile to another machine")
...
### Permission denied when reloading shell after install[​](<#permission-denied-when-reloading-shell-after-install> "Direct link to Permission denied when reloading shell after install")
...
[/code]
### Error 400 on first agent run[​](<#error-400-on-first-agent-run> "Direct link to Error 400 on first agent run")
...
If your issue isn't covered here:
  ...
  * [Frequently Asked Questions](<#frequently-asked-questions>)
    ...
    * [I run Hermes in WSL2. What's the best way to control my normal Windows Chrome?](<#i-run-hermes-in-wsl2-whats-the-best-way-to-control-my-normal-windows-chrome>)
  ...
  * [Workflows & Patterns](<#workflows--patterns>)
    ...
    * [Moving a single profile to another machine](<#moving-a-single-profile-to-another-machine>)
