...
## Live manifest URL[​](<#live-manifest-url> "Direct link to Live manifest URL")
[code] 
...
### Per-provider override URLs[​](<#per-provider-override-urls> "Direct link to Per-provider override URLs")
...
## Updating the manifest[​](<#updating-the-manifest> "Direct link to Updating the manifest")
Maintainers:
[code] 
...
You can also hand-edit the JSON directly for fine-grained metadata changes that don't belong in the in-repo snapshot — the generator script is a convenience, not the single source of truth.
  ...
  * [Config](<#config>)
    * [Per-provider override URLs](<#per-provider-override-urls>)
