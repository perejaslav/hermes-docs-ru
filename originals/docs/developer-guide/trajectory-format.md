...
## JSONL Entry Format[​](<#jsonl-entry-format> "Direct link to JSONL Entry Format")
Each line in the file is a self-contained JSON object. There are two variants:
### CLI/Interactive Format (from `_save_trajectory`)[​](<#cliinteractive-format-from-_save_trajectory> "Direct link to cliinteractive-format-from-_save_trajectory")
[code] 
    {  
      "conversations": [ ... ],  
...
[/code]
### Batch Runner Format (from `batch_runner.py`)[​](<#batch-runner-format-from-batch_runnerpy> "Direct link to batch-runner-format-from-batch_runnerpy")
[code] 
    {  
      "prompt_index": 42,  
      "conversations": [ ... ],  
...
## Conversations Array (ShareGPT Format)[​](<#conversations-array-sharegpt-format> "Direct link to Conversations Array \(ShareGPT Format\)")
...
[/code]
## Normalization Rules[​](<#normalization-rules> "Direct link to Normalization Rules")
### Reasoning Content Markup[​](<#reasoning-content-markup> "Direct link to Reasoning Content Markup")
The trajectory converter normalizes ALL reasoning into `<think>` tags, regardless of how the model originally produced it:
  ...
  2. **REASONING_SCRATCHPAD XML** (when native thinking is disabled and the model reasons via system-prompt-instructed XML): `<REASONING_SCRATCHPAD>` tags are converted to `<think>` via `convert_scratchpad_to_think()`.
  3. **Empty think blocks** : Every `gpt` turn is guaranteed to have a `<think>` block. If no reasoning was produced, an empty block is inserted: `<think>\n</think>\n` — this ensures consistent format for training data.


### Tool Call Normalization[​](<#tool-call-normalization> "Direct link to Tool Call Normalization")
...
### Tool Response Normalization[​](<#tool-response-normalization> "Direct link to Tool Response Normalization")
...
## Loading Trajectories[​](<#loading-trajectories> "Direct link to Loading Trajectories")
...
[/code]
### Loading for HuggingFace Datasets[​](<#loading-for-huggingface-datasets> "Direct link to Loading for HuggingFace Datasets")
[code] 
...
## Controlling Trajectory Saving[​](<#controlling-trajectory-saving> "Direct link to Controlling Trajectory Saving")
...
Samples with zero reasoning across all turns are automatically discarded by the batch runner to avoid polluting training data with non-reasoning examples.
  ...
  * [JSONL Entry Format](<#jsonl-entry-format>)
    * [CLI/Interactive Format (from `_save_trajectory`)](<#cliinteractive-format-from-_save_trajectory>)
    * [Batch Runner Format (from `batch_runner.py`)](<#batch-runner-format-from-batch_runnerpy>)
  * [Conversations Array (ShareGPT Format)](<#conversations-array-sharegpt-format>)
    * [Complete Example](<#complete-example>)
  * [Normalization Rules](<#normalization-rules>)
    * [Reasoning Content Markup](<#reasoning-content-markup>)
    * [Tool Call Normalization](<#tool-call-normalization>)
    * [Tool Response Normalization](<#tool-response-normalization>)
    * [System Message](<#system-message>)
  * [Loading Trajectories](<#loading-trajectories>)
    * [Loading for HuggingFace Datasets](<#loading-for-huggingface-datasets>)
  * [Controlling Trajectory Saving](<#controlling-trajectory-saving>)
