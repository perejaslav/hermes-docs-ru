...
## Why WSL2 (and not "just Windows")[​](<#why-wsl2-and-not-just-windows> "Direct link to Why WSL2 \(and not "just Windows"\)")
...
## Install WSL2[​](<#install-wsl2> "Direct link to Install WSL2")
...
### Distro choice[​](<#distro-choice> "Direct link to Distro choice")
...
### Enable systemd (recommended)[​](<#enable-systemd-recommended> "Direct link to Enable systemd \(recommended\)")
...
### Install Hermes inside WSL[​](<#install-hermes-inside-wsl> "Direct link to Install Hermes inside WSL")
...
## Filesystem: crossing the Windows ↔ WSL2 boundary[​](<#filesystem-crossing-the-windows--wsl2-boundary> "Direct link to Filesystem: crossing the Windows ↔ WSL2 boundary")
...
### The two directions[​](<#the-two-directions> "Direct link to The two directions")
...
### Where to put Hermes and your projects[​](<#where-to-put-hermes-and-your-projects> "Direct link to Where to put Hermes and your projects")
...
### Getting files back and forth[​](<#getting-files-back-and-forth> "Direct link to Getting files back and forth")
...
[/code]
### Line endings, BOMs, and git[​](<#line-endings-boms-and-git> "Direct link to Line endings, BOMs, and git")
...
[/code]
### "Clone inside WSL or on `/mnt/c`?"[​](<#clone-inside-wsl-or-on-mntc> "Direct link to clone-inside-wsl-or-on-mntc")
...
## Networking: WSL ↔ Windows[​](<#networking-wsl--windows> "Direct link to Networking: WSL ↔ Windows")
...
### Case 1 — Hermes in WSL talks to a service on Windows[​](<#case-1--hermes-in-wsl-talks-to-a-service-on-windows> "Direct link to Case 1 — Hermes in WSL talks to a service on Windows")
...
### Case 2 — Something on Windows (or your LAN) talks to Hermes in WSL[​](<#case-2--something-on-windows-or-your-lan-talks-to-hermes-in-wsl> "Direct link to Case 2 — Something on Windows \(or your LAN\) talks to Hermes in WSL")
...
#### Subcase 2a: from the Windows host itself[​](<#subcase-2a-from-the-windows-host-itself> "Direct link to Subcase 2a: from the Windows host itself")
...
#### Subcase 2b: from another device on your LAN (phone, tablet, another PC)[​](<#subcase-2b-from-another-device-on-your-lan-phone-tablet-another-pc> "Direct link to Subcase 2b: from another device on your LAN \(phone, tablet, another PC\)")
...
[code] # Grab the WSL VM's current IP (it changes on every WSL restart under NAT)  
...
## Running Hermes services long-term on Windows[​](<#running-hermes-services-long-term-on-windows> "Direct link to Running Hermes services long-term on Windows")
...
### Inside WSL with systemd (recommended)[​](<#inside-wsl-with-systemd-recommended> "Direct link to Inside WSL with systemd \(recommended\)")
...
### Making WSL itself start on Windows login[​](<#making-wsl-itself-start-on-windows-login> "Direct link to Making WSL itself start on Windows login")
...
## GPU passthrough (local models)[​](<#gpu-passthrough-local-models> "Direct link to GPU passthrough \(local models\)")
...
**"Connection refused" to my Windows-hosted Ollama / LM Studio.** See [WSL2 Networking](</docs/integrations/providers#wsl2-networking-windows-users>). Ninety percent of the time the server is bound to `127.0.0.1` and needs `0.0.0.0` (Ollama: `OLLAMA_HOST=0.0.0.0`), or you're missing a firewall rule.
...
## Where to go next[​](<#where-to-go-next> "Direct link to Where to go next")
  ...
  * [Why WSL2 (and not "just Windows")](<#why-wsl2-and-not-just-windows>)
  * [Install WSL2](<#install-wsl2>)
    * [Distro choice](<#distro-choice>)
    * [Enable systemd (recommended)](<#enable-systemd-recommended>)
    * [Install Hermes inside WSL](<#install-hermes-inside-wsl>)
  * [Filesystem: crossing the Windows ↔ WSL2 boundary](<#filesystem-crossing-the-windows--wsl2-boundary>)
    * [The two directions](<#the-two-directions>)
    * [Where to put Hermes and your projects](<#where-to-put-hermes-and-your-projects>)
    * [Getting files back and forth](<#getting-files-back-and-forth>)
    * [Line endings, BOMs, and git](<#line-endings-boms-and-git>)
    * ["Clone inside WSL or on `/mnt/c`?"](<#clone-inside-wsl-or-on-mntc>)
  * [Networking: WSL ↔ Windows](<#networking-wsl--windows>)
    * [Case 1 — Hermes in WSL talks to a service on Windows](<#case-1--hermes-in-wsl-talks-to-a-service-on-windows>)
    * [Case 2 — Something on Windows (or your LAN) talks to Hermes in WSL](<#case-2--something-on-windows-or-your-lan-talks-to-hermes-in-wsl>)
  * [Running Hermes services long-term on Windows](<#running-hermes-services-long-term-on-windows>)
    * [Inside WSL with systemd (recommended)](<#inside-wsl-with-systemd-recommended>)
    * [Making WSL itself start on Windows login](<#making-wsl-itself-start-on-windows-login>)
  * [GPU passthrough (local models)](<#gpu-passthrough-local-models>)
