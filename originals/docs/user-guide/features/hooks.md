...
## Gateway Event Hooks[​](<#gateway-event-hooks> "Direct link to Gateway Event Hooks")
...
### Creating a Hook[​](<#creating-a-hook> "Direct link to Creating a Hook")
...
[/code]
#### HOOK.yaml[​](<#hookyaml> "Direct link to HOOK.yaml")
[code] 
    name: my-hook  
...
#### handler.py[​](<#handlerpy> "Direct link to handler.py")
[code] 
    import json  
...
#### Wildcard Matching[​](<#wildcard-matching> "Direct link to Wildcard Matching")
Handlers registered for `command:*` fire for any `command:` event (`command:model`, `command:reset`, etc.). Monitor all slash commands with a single subscription.
### Examples[​](<#examples> "Direct link to Examples")
#### Telegram Alert on Long Tasks[​](<#telegram-alert-on-long-tasks> "Direct link to Telegram Alert on Long Tasks")
...
[/code]
#### Command Usage Logger[​](<#command-usage-logger> "Direct link to Command Usage Logger")
...
[/code]
#### Session Start Webhook[​](<#session-start-webhook> "Direct link to Session Start Webhook")
...
[/code]
### Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot[​](<#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot> "Direct link to Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot")
...
#### Step 1: Write your checklist[​](<#step-1-write-your-checklist> "Direct link to Step 1: Write your checklist")
...
#### Step 2: Create the hook[​](<#step-2-create-the-hook> "Direct link to Step 2: Create the hook")
[code] 
...
#### Extending the pattern[​](<#extending-the-pattern> "Direct link to Extending the pattern")
...
#### Why this isn't a built-in[​](<#why-this-isnt-a-built-in> "Direct link to Why this isn't a built-in")
...
## Plugin Hooks[​](<#plugin-hooks> "Direct link to Plugin Hooks")
...
* * *
### `pre_tool_call`[​](<#pre_tool_call> "Direct link to pre_tool_call")
...
[/code]
* * *
### `post_tool_call`[​](<#post_tool_call> "Direct link to post_tool_call")
...
[/code]
* * *
### `pre_llm_call`[​](<#pre_llm_call> "Direct link to pre_llm_call")
...
[/code]
* * *
### `post_llm_call`[​](<#post_llm_call> "Direct link to post_llm_call")
...
[/code]
* * *
### `on_session_start`[​](<#on_session_start> "Direct link to on_session_start")
...
[/code]
* * *
### `on_session_end`[​](<#on_session_end> "Direct link to on_session_end")
...
[/code]
* * *
### `on_session_finalize`[​](<#on_session_finalize> "Direct link to on_session_finalize")
...
* * *
### `on_session_reset`[​](<#on_session_reset> "Direct link to on_session_reset")
...
* * *
### `subagent_stop`[​](<#subagent_stop> "Direct link to subagent_stop")
...
* * *
### `pre_gateway_dispatch`[​](<#pre_gateway_dispatch> "Direct link to pre_gateway_dispatch")
...
[/code]
* * *
### `pre_approval_request`[​](<#pre_approval_request> "Direct link to pre_approval_request")
...
[/code]
* * *
### `post_approval_response`[​](<#post_approval_response> "Direct link to post_approval_response")
...
[/code]
* * *
### `transform_tool_result`[​](<#transform_tool_result> "Direct link to transform_tool_result")
...
* * *
### `transform_terminal_output`[​](<#transform_terminal_output> "Direct link to transform_terminal_output")
...
* * *
## Shell Hooks[​](<#shell-hooks> "Direct link to Shell Hooks")
...
### Configuration schema[​](<#configuration-schema> "Direct link to Configuration schema")
[code] 
    hooks:  
...
### JSON wire protocol[​](<#json-wire-protocol> "Direct link to JSON wire protocol")
...
#### 1\. Auto-format Python files after every write[​](<#1-auto-format-python-files-after-every-write> "Direct link to 1. Auto-format Python files after every write")
[code] 
    # ~/.hermes/config.yaml  
    hooks:  
      post_tool_call:  
...
#### 2\. Block destructive `terminal` commands[​](<#2-block-destructive-terminal-commands> "Direct link to 2-block-destructive-terminal-commands")
[code] 
    hooks:  
      pre_tool_call:  
        - matcher: "terminal"  
...
[/code]
#### 3\. Inject `git status` into every turn (Claude-Code `UserPromptSubmit` equivalent)[​](<#3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent> "Direct link to 3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent")
[code] 
    hooks:  
      pre_llm_call:  
...
#### 4\. Log every subagent completion[​](<#4-log-every-subagent-completion> "Direct link to 4. Log every subagent completion")
[code] 
    hooks:  
      subagent_stop:  
...
[/code]
### Consent model[​](<#consent-model> "Direct link to Consent model")
...
### The `hermes hooks` CLI[​](<#the-hermes-hooks-cli> "Direct link to the-hermes-hooks-cli")
...
### Ordering and precedence[​](<#ordering-and-precedence> "Direct link to Ordering and precedence")
Both Python plugin hooks and shell hooks flow through the same `invoke_hook()` dispatcher. Python plugins are registered first (`discover_and_load()`), shell hooks second (`register_from_config()`), so Python `pre_tool_call` block decisions take precedence in tie cases. The first valid block wins — the aggregator returns as soon as any callback produces `{"action": "block", "message": str}` with a non-empty message.
  * [Gateway Event Hooks](<#gateway-event-hooks>)
    ...
    * [Examples](<#examples>)
    * [Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot](<#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot>)
  ...
  * [Shell Hooks](<#shell-hooks>)
    ...
    * [The `hermes hooks` CLI](<#the-hermes-hooks-cli>)
    * [Security](<#security>)
