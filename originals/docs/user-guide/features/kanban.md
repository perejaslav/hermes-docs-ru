...
## Kanban vs. `delegate_task`[​](<#kanban-vs-delegate_task> "Direct link to kanban-vs-delegate_task")
...
## Core concepts[​](<#core-concepts> "Direct link to Core concepts")
  ...
  * **Tenant** — optional string namespace _within_ a board. One specialist fleet can serve multiple businesses (`--tenant business-a`) with data isolation by workspace path and memory key prefix. Tenants are a soft filter; boards are the hard isolation boundary.


## Boards (multi-project)[​](<#boards-multi-project> "Direct link to Boards \(multi-project\)")
...
### Managing boards from the CLI[​](<#managing-boards-from-the-cli> "Direct link to Managing boards from the CLI")
[code] 
...
### Managing boards from the dashboard[​](<#managing-boards-from-the-dashboard> "Direct link to Managing boards from the dashboard")
...
### Gateway-embedded dispatcher (default)[​](<#gateway-embedded-dispatcher-default> "Direct link to Gateway-embedded dispatcher \(default\)")
...
### Idempotent create (for automation / webhooks)[​](<#idempotent-create-for-automation--webhooks> "Direct link to Idempotent create \(for automation / webhooks\)")
[code] 
...
[/code]
### Bulk CLI verbs[​](<#bulk-cli-verbs> "Direct link to Bulk CLI verbs")
...
[/code]
## How workers interact with the board[​](<#how-workers-interact-with-the-board> "Direct link to How workers interact with the board")
...
### Recommended handoff evidence[​](<#recommended-handoff-evidence> "Direct link to Recommended handoff evidence")
...
### The worker skill[​](<#the-worker-skill> "Direct link to The worker skill")
...
### Pinning extra skills to a specific task[​](<#pinning-extra-skills-to-a-specific-task> "Direct link to Pinning extra skills to a specific task")
...
### The orchestrator skill[​](<#the-orchestrator-skill> "Direct link to The orchestrator skill")
...
## Dashboard (GUI)[​](<#dashboard-gui> "Direct link to Dashboard \(GUI\)")
...
[/code]
### What the plugin gives you[​](<#what-the-plugin-gives-you> "Direct link to What the plugin gives you")
...
[/code]
### REST surface[​](<#rest-surface> "Direct link to REST surface")
...
### Dashboard config[​](<#dashboard-config> "Direct link to Dashboard config")
...
### Live updates[​](<#live-updates> "Direct link to Live updates")
...
### Extending it[​](<#extending-it> "Direct link to Extending it")
...
### Scope boundary[​](<#scope-boundary> "Direct link to Scope boundary")
...
## `/kanban` slash command[​](<#kanban-slash-command> "Direct link to kanban-slash-command")
...
### Mid-run usage: `/kanban` bypasses the running-agent guard[​](<#mid-run-usage-kanban-bypasses-the-running-agent-guard> "Direct link to mid-run-usage-kanban-bypasses-the-running-agent-guard")
...
This is the whole point of the separation:
  ...
  * You spot a card that needs human context → `/kanban comment t_xyz "use the 2026 schema, not 2025"` lands on the task thread and the _next_ run of that task will read it in `kanban_show()`.
...
### Auto-subscribe on `/kanban create` (gateway only)[​](<#auto-subscribe-on-kanban-create-gateway-only> "Direct link to auto-subscribe-on-kanban-create-gateway-only")
...
### Output truncation in messaging[​](<#output-truncation-in-messaging> "Direct link to Output truncation in messaging")
...
## Collaboration patterns[​](<#collaboration-patterns> "Direct link to Collaboration patterns")
...
## Multi-tenant usage[​](<#multi-tenant-usage> "Direct link to Multi-tenant usage")
...
## Gateway notifications[​](<#gateway-notifications> "Direct link to Gateway notifications")
...
## Runs — one row per attempt[​](<#runs--one-row-per-attempt> "Direct link to Runs — one row per attempt")
...
**Bulk close caveat.** `hermes kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close _without_ `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
### Forward compatibility[​](<#forward-compatibility> "Direct link to Forward compatibility")
...
## Design spec[​](<#design-spec> "Direct link to Design spec")
The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/hermes-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
  * [Two surfaces: the model talks through tools, you talk through the CLI](<#two-surfaces-the-model-talks-through-tools-you-talk-through-the-cli>)
  * [Kanban vs. `delegate_task`](<#kanban-vs-delegate_task>)
  * [Core concepts](<#core-concepts>)
  * [Boards (multi-project)](<#boards-multi-project>)
    * [Managing boards from the CLI](<#managing-boards-from-the-cli>)
    * [Managing boards from the dashboard](<#managing-boards-from-the-dashboard>)
  * [Quick start](<#quick-start>)
    * [Gateway-embedded dispatcher (default)](<#gateway-embedded-dispatcher-default>)
    * [Idempotent create (for automation / webhooks)](<#idempotent-create-for-automation--webhooks>)
    * [Bulk CLI verbs](<#bulk-cli-verbs>)
  * [How workers interact with the board](<#how-workers-interact-with-the-board>)
    ...
    * [Recommended handoff evidence](<#recommended-handoff-evidence>)
    * [The worker skill](<#the-worker-skill>)
    * [Pinning extra skills to a specific task](<#pinning-extra-skills-to-a-specific-task>)
    * [The orchestrator skill](<#the-orchestrator-skill>)
  ...
  * [`/kanban` slash command](<#kanban-slash-command>)
    * [Mid-run usage: `/kanban` bypasses the running-agent guard](<#mid-run-usage-kanban-bypasses-the-running-agent-guard>)
    * [Auto-subscribe on `/kanban create` (gateway only)](<#auto-subscribe-on-kanban-create-gateway-only>)
    * [Output truncation in messaging](<#output-truncation-in-messaging>)
    * [Autocomplete](<#autocomplete>)
  * [Collaboration patterns](<#collaboration-patterns>)
  * [Multi-tenant usage](<#multi-tenant-usage>)
  * [Gateway notifications](<#gateway-notifications>)
  * [Runs — one row per attempt](<#runs--one-row-per-attempt>)
