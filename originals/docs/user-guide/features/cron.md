...
## No-agent mode (script-only jobs)[​](<#no-agent-mode-script-only-jobs> "Direct link to No-agent mode \(script-only jobs\)")
...
### The agent sets these up for you[​](<#the-agent-sets-these-up-for-you> "Direct link to The agent sets these up for you")
...
## Chaining jobs with `context_from`[​](<#chaining-jobs-with-context_from> "Direct link to chaining-jobs-with-context_from")
...
## Toolsets available to cron jobs[​](<#toolsets-available-to-cron-jobs> "Direct link to Toolsets available to cron jobs")
...
### Skipping the agent entirely: `wakeAgent`[​](<#skipping-the-agent-entirely-wakeagent> "Direct link to skipping-the-agent-entirely-wakeagent")
...
### Chaining jobs: `context_from`[​](<#chaining-jobs-context_from> "Direct link to chaining-jobs-context_from")
...
Scheduled task prompts are scanned for prompt-injection and credential-exfiltration patterns at creation and update time. Prompts containing invisible Unicode tricks, SSH backdoor attempts, or obvious secret-exfiltration payloads are blocked.
  * [What cron can do now](<#what-cron-can-do-now>)
  * [Creating scheduled tasks](<#creating-scheduled-tasks>)
    * [In chat with `/cron`](<#in-chat-with-cron>)
    * [From the standalone CLI](<#from-the-standalone-cli>)
    * [Through natural conversation](<#through-natural-conversation>)
  * [Skill-backed cron jobs](<#skill-backed-cron-jobs>)
    * [Single skill](<#single-skill>)
    * [Multiple skills](<#multiple-skills>)
  * [Running a job inside a project directory](<#running-a-job-inside-a-project-directory>)
  ...
  * [How it works](<#how-it-works>)
    * [Gateway scheduler behavior](<#gateway-scheduler-behavior>)
  * [Delivery options](<#delivery-options>)
    * [Response wrapping](<#response-wrapping>)
    * [Silent suppression](<#silent-suppression>)
  * [Script timeout](<#script-timeout>)
  * [No-agent mode (script-only jobs)](<#no-agent-mode-script-only-jobs>)
    * [The agent sets these up for you](<#the-agent-sets-these-up-for-you>)
  * [Chaining jobs with `context_from`](<#chaining-jobs-with-context_from>)
  * [Provider recovery](<#provider-recovery>)
  * [Schedule formats](<#schedule-formats>)
    * [Relative delays (one-shot)](<#relative-delays-one-shot>)
    * [Intervals (recurring)](<#intervals-recurring>)
    * [Cron expressions](<#cron-expressions>)
    * [ISO timestamps](<#iso-timestamps>)
  * [Repeat behavior](<#repeat-behavior>)
  * [Managing jobs programmatically](<#managing-jobs-programmatically>)
  * [Toolsets available to cron jobs](<#toolsets-available-to-cron-jobs>)
    * [Skipping the agent entirely: `wakeAgent`](<#skipping-the-agent-entirely-wakeagent>)
    * [Chaining jobs: `context_from`](<#chaining-jobs-context_from>)
