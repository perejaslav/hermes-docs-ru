...
## Field Types (request body shapes)[​](<#field-types-request-body-shapes> "Direct link to Field Types \(request body shapes\)")
...
### List bases the token can see[​](<#list-bases-the-token-can-see> "Direct link to List bases the token can see")
[code] 
    curl -s "https://api.airtable.com/v0/meta/bases" \  
      -H "Authorization: Bearer $AIRTABLE_API_KEY" | python3 -m json.tool  
    
[/code]
### List tables + schema for a base[​](<#list-tables--schema-for-a-base> "Direct link to List tables + schema for a base")
[code] 
...
### List records (first 10)[​](<#list-records-first-10> "Direct link to List records \(first 10\)")
[code] 
    curl -s "https://api.airtable.com/v0/$BASE_ID/$TABLE?maxRecords=10" \  
      -H "Authorization: Bearer $AIRTABLE_API_KEY" | python3 -m json.tool  
    
[/code]
### Get a single record[​](<#get-a-single-record> "Direct link to Get a single record")
[code] 
    curl -s "https://api.airtable.com/v0/$BASE_ID/$TABLE/$RECORD_ID" \  
      -H "Authorization: Bearer $AIRTABLE_API_KEY" | python3 -m json.tool  
    
[/code]
### Filter records (filterByFormula)[​](<#filter-records-filterbyformula> "Direct link to Filter records \(filterByFormula\)")
...
### Sort + select specific fields[​](<#sort--select-specific-fields> "Direct link to Sort + select specific fields")
[code] 
...
### Use a named view[​](<#use-a-named-view> "Direct link to Use a named view")
[code] 
...
[/code]
### Create up to 10 records in one call[​](<#create-up-to-10-records-in-one-call> "Direct link to Create up to 10 records in one call")
[code] 
    curl -s -X POST "https://api.airtable.com/v0/$BASE_ID/$TABLE" \  
      -H "Authorization: Bearer $AIRTABLE_API_KEY" \  
      -H "Content-Type: application/json" \  
...
### Update a record (PATCH — merges, preserves unchanged fields)[​](<#update-a-record-patch--merges-preserves-unchanged-fields> "Direct link to Update a record \(PATCH — merges, preserves unchanged fields\)")
[code] 
...
[/code]
### Upsert by a merge field (no ID needed)[​](<#upsert-by-a-merge-field-no-id-needed> "Direct link to Upsert by a merge field \(no ID needed\)")
[code] 
...
### Delete a record[​](<#delete-a-record> "Direct link to Delete a record")
[code] 
    curl -s -X DELETE "https://api.airtable.com/v0/$BASE_ID/$TABLE/$RECORD_ID" \  
      -H "Authorization: Bearer $AIRTABLE_API_KEY" | python3 -m json.tool  
    
[/code]
### Delete up to 10 records in one call[​](<#delete-up-to-10-records-in-one-call> "Direct link to Delete up to 10 records in one call")
[code] 
...
[/code]
## Typical Hermes Workflow[​](<#typical-hermes-workflow> "Direct link to Typical Hermes Workflow")
...
## Important Notes for Hermes[​](<#important-notes-for-hermes> "Direct link to Important Notes for Hermes")
  ...
  * [Field Types (request body shapes)](<#field-types-request-body-shapes>)
  * [Common Queries](<#common-queries>)
    * [List bases the token can see](<#list-bases-the-token-can-see>)
    * [List tables + schema for a base](<#list-tables--schema-for-a-base>)
    * [List records (first 10)](<#list-records-first-10>)
    * [Get a single record](<#get-a-single-record>)
    * [Filter records (filterByFormula)](<#filter-records-filterbyformula>)
    * [Sort + select specific fields](<#sort--select-specific-fields>)
    * [Use a named view](<#use-a-named-view>)
  * [Common Mutations](<#common-mutations>)
    * [Create a record](<#create-a-record>)
    * [Create up to 10 records in one call](<#create-up-to-10-records-in-one-call>)
    * [Update a record (PATCH — merges, preserves unchanged fields)](<#update-a-record-patch--merges-preserves-unchanged-fields>)
    * [Upsert by a merge field (no ID needed)](<#upsert-by-a-merge-field-no-id-needed>)
    * [Delete a record](<#delete-a-record>)
    * [Delete up to 10 records in one call](<#delete-up-to-10-records-in-one-call>)
  * [Pagination](<#pagination>)
  * [Typical Hermes Workflow](<#typical-hermes-workflow>)
  * [Pitfalls](<#pitfalls>)
  * [Important Notes for Hermes](<#important-notes-for-hermes>)
