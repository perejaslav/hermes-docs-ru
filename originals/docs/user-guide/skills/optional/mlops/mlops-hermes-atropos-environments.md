...
## config_init() — Default Configuration[​](<#config_init--default-configuration> "Direct link to config_init\(\) — Default Configuration")
...
[/code]
  ...
  * [Inference Setup — Ask the User First](<#inference-setup--ask-the-user-first>)
    * [Key flags by provider:](<#key-flags-by-provider>)
  * [Required Methods](<#required-methods>)
    * [1\. `setup()` — Load dataset and initialize state](<#1-setup--load-dataset-and-initialize-state>)
    * [2\. `get_next_item()` — Return next training item](<#2-get_next_item--return-next-training-item>)
    * [3\. `format_prompt(item)` — Convert item to user message](<#3-format_promptitem--convert-item-to-user-message>)
    * [4\. `compute_reward(item, result, ctx)` — Score the rollout](<#4-compute_rewarditem-result-ctx--score-the-rollout>)
    * [5\. `evaluate()` — Periodic evaluation with full agent loop](<#5-evaluate--periodic-evaluation-with-full-agent-loop>)
    * [6\. `wandb_log()` — Custom metrics logging](<#6-wandb_log--custom-metrics-logging>)
  * [Config Class](<#config-class>)
  * [config_init() — Default Configuration](<#config_init--default-configuration>)
  * [Three CLI Modes](<#three-cli-modes>)
  * [Common Pitfalls](<#common-pitfalls>)
  * [Reward Function Patterns](<#reward-function-patterns>)
    * [LLM Judge (for open-ended tasks)](<#llm-judge-for-open-ended-tasks>)
    * [Binary Verification (for code/terminal tasks)](<#binary-verification-for-codeterminal-tasks>)
    * [Multi-Signal (combine multiple indicators)](<#multi-signal-combine-multiple-indicators>)
...
<!-- Source: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/mlops/mlops-hermes-atropos-environments -->
