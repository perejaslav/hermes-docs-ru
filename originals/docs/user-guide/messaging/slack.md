...
### Option A: From a Hermes-generated manifest (recommended)[​](<#option-a-from-a-hermes-generated-manifest-recommended> "Direct link to Option A: From a Hermes-generated manifest \(recommended\)")
...
### Option B: From scratch (manual)[​](<#option-b-from-scratch-manual> "Direct link to Option B: From scratch \(manual\)")
...
* * *
## Step 2: Configure Bot Token Scopes[​](<#step-2-configure-bot-token-scopes> "Direct link to Step 2: Configure Bot Token Scopes")
...
* * *
## Step 3: Enable Socket Mode[​](<#step-3-enable-socket-mode> "Direct link to Step 3: Enable Socket Mode")
...
* * *
## Step 4: Subscribe to Events[​](<#step-4-subscribe-to-events> "Direct link to Step 4: Subscribe to Events")
...
* * *
## Step 5: Enable the Messages Tab[​](<#step-5-enable-the-messages-tab> "Direct link to Step 5: Enable the Messages Tab")
...
* * *
## Step 6: Install App to Workspace[​](<#step-6-install-app-to-workspace> "Direct link to Step 6: Install App to Workspace")
...
* * *
## Step 7: Find User IDs for the Allowlist[​](<#step-7-find-user-ids-for-the-allowlist> "Direct link to Step 7: Find User IDs for the Allowlist")
...
* * *
## Step 8: Configure Hermes[​](<#step-8-configure-hermes> "Direct link to Step 8: Configure Hermes")
...
[/code]
* * *
## Step 9: Invite the Bot to Channels[​](<#step-9-invite-the-bot-to-channels> "Direct link to Step 9: Invite the Bot to Channels")
...
Under the hood: Hermes ships with a generated Slack app manifest (see Step 1, Option A) that declares every command in [`COMMAND_REGISTRY`](<https://github.com/NousResearch/hermes-agent/blob/main/hermes_cli/commands.py>) as a slash command. In Socket Mode, Slack routes the command event through the WebSocket regardless of the manifest's `url` field.
### Refreshing slash commands after updates[​](<#refreshing-slash-commands-after-updates> "Direct link to Refreshing slash commands after updates")
...
[/code]
Then in Slack:
  1. Open <https://api.slack.com/apps> → your Hermes app
...
### Legacy `/hermes <subcommand>` still works[​](<#legacy-hermes-subcommand-still-works> "Direct link to legacy-hermes-subcommand-still-works")
...
### Advanced: emit only the slash-commands array[​](<#advanced-emit-only-the-slash-commands-array> "Direct link to Advanced: emit only the slash-commands array")
...
### Thread & Reply Behavior[​](<#thread--reply-behavior> "Direct link to Thread & Reply Behavior")
[code] 
    platforms:  
      slack:  
...
### Mention & Trigger Behavior[​](<#mention--trigger-behavior> "Direct link to Mention & Trigger Behavior")
[code] 
    slack:  
...
### Unauthorized User Handling[​](<#unauthorized-user-handling> "Direct link to Unauthorized User Handling")
[code] 
    slack:  
...
* * *
## Multi-Workspace Support[​](<#multi-workspace-support> "Direct link to Multi-Workspace Support")
...
[/code]
### OAuth Token File[​](<#oauth-token-file> "Direct link to OAuth Token File")
...
## Per-Channel Skill Bindings[​](<#per-channel-skill-bindings> "Direct link to Per-Channel Skill Bindings")
...
**Always set`SLACK_ALLOWED_USERS`** with the Member IDs of authorized users. Without this setting, the gateway will **deny all messages** by default as a safety measure. Never share your bot tokens — treat them like passwords.
  ...
  * [Step 1: Create a Slack App](<#step-1-create-a-slack-app>)
    * [Option A: From a Hermes-generated manifest (recommended)](<#option-a-from-a-hermes-generated-manifest-recommended>)
    * [Option B: From scratch (manual)](<#option-b-from-scratch-manual>)
  ...
  * [Slash Commands](<#slash-commands>)
    * [Refreshing slash commands after updates](<#refreshing-slash-commands-after-updates>)
    * [Legacy `/hermes <subcommand>` still works](<#legacy-hermes-subcommand-still-works>)
    * [Advanced: emit only the slash-commands array](<#advanced-emit-only-the-slash-commands-array>)
