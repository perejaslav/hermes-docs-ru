...
* * *
## Step 1: Run the Setup Wizard[​](<#step-1-run-the-setup-wizard> "Direct link to Step 1: Run the Setup Wizard")
[code] 
    hermes whatsapp  
...
* * *
## Step 2: Getting a Second Phone Number (Bot Mode)[​](<#step-2-getting-a-second-phone-number-bot-mode> "Direct link to Step 2: Getting a Second Phone Number \(Bot Mode\)")
...
[/code]
Allow-all shorthand
Setting `WHATSAPP_ALLOWED_USERS=*` allows **all** senders (equivalent to `WHATSAPP_ALLOW_ALL_USERS=true`). This is consistent with [Signal group allowlists](</docs/reference/environment-variables>). To use the pairing flow instead, remove both variables and rely on the [DM pairing system](</docs/user-guide/security#dm-pairing-system>).
...
* * *
## Re-pairing[​](<#re-pairing> "Direct link to Re-pairing")
...
[/code]
* * *
## Message Formatting & Delivery[​](<#message-formatting--delivery> "Direct link to Message Formatting & Delivery")
...
### WhatsApp-Compatible Markdown[​](<#whatsapp-compatible-markdown> "Direct link to WhatsApp-Compatible Markdown")
...
### Tool Progress[​](<#tool-progress> "Direct link to Tool Progress")
...
**macOS: "Node.js not installed" but node works in terminal**|  launchd services don't inherit your shell PATH. Run `hermes gateway install` to re-snapshot your current PATH into the plist, then `hermes gateway start`. See the [Gateway Service docs](</docs/user-guide/messaging/#macos-launchd>) for details.  
