...
## How Hermes Behaves[​](<#how-hermes-behaves> "Direct link to How Hermes Behaves")
Context| Behavior  
---|---  
...
### Session Model in Matrix[​](<#session-model-in-matrix> "Direct link to Session Model in Matrix")
By default:
...
### Mention and Threading Configuration[​](<#mention-and-threading-configuration> "Direct link to Mention and Threading Configuration")
...
## Step 1: Create a Bot Account[​](<#step-1-create-a-bot-account> "Direct link to Step 1: Create a Bot Account")
You need a Matrix user account for the bot. There are several ways to do this:
### Option A: Register on Your Homeserver (Recommended)[​](<#option-a-register-on-your-homeserver-recommended> "Direct link to Option A: Register on Your Homeserver \(Recommended\)")
...
### Option B: Use matrix.org or Another Public Homeserver[​](<#option-b-use-matrixorg-or-another-public-homeserver> "Direct link to Option B: Use matrix.org or Another Public Homeserver")
  1. Go to [Element Web](<https://app.element.io>) and create a new account.
  2. Pick a username for your bot (e.g., `hermes-bot`).


### Option C: Use Your Own Account[​](<#option-c-use-your-own-account> "Direct link to Option C: Use Your Own Account")
You can also run Hermes as your own user. This means the bot posts as you — useful for personal assistants.
## Step 2: Get an Access Token[​](<#step-2-get-an-access-token> "Direct link to Step 2: Get an Access Token")
Hermes needs an access token to authenticate with the homeserver. You have two options:
### Option A: Access Token (Recommended)[​](<#option-a-access-token-recommended> "Direct link to Option A: Access Token \(Recommended\)")
...
### Option B: Password Login[​](<#option-b-password-login> "Direct link to Option B: Password Login")
...
[/code]
## Step 3: Find Your Matrix User ID[​](<#step-3-find-your-matrix-user-id> "Direct link to Step 3: Find Your Matrix User ID")
...
## Step 4: Configure Hermes Agent[​](<#step-4-configure-hermes-agent> "Direct link to Step 4: Configure Hermes Agent")
...
## End-to-End Encryption (E2EE)[​](<#end-to-end-encryption-e2ee> "Direct link to End-to-End Encryption \(E2EE\)")
...
[/code]
### Enable E2EE[​](<#enable-e2ee> "Direct link to Enable E2EE")
...
### Cross-Signing Verification (Recommended)[​](<#cross-signing-verification-recommended> "Direct link to Cross-Signing Verification \(Recommended\)")
...
## Home Room[​](<#home-room> "Direct link to Home Room")
...
### Using the Slash Command[​](<#using-the-slash-command> "Direct link to Using the Slash Command")
...
### Bot is not responding to messages[​](<#bot-is-not-responding-to-messages> "Direct link to Bot is not responding to messages")
...
### "Failed to authenticate" / "whoami failed" on startup[​](<#failed-to-authenticate--whoami-failed-on-startup> "Direct link to "Failed to authenticate" / "whoami failed" on startup")
...
### "mautrix not installed" error[​](<#mautrix-not-installed-error> "Direct link to "mautrix not installed" error")
...
[/code]
### Encryption errors / "could not decrypt event"[​](<#encryption-errors--could-not-decrypt-event> "Direct link to Encryption errors / "could not decrypt event"")
...
**Fix** :
  ...
  4. If the bot just joined an encrypted room, it can only decrypt messages sent _after_ it joined. Older messages are inaccessible.


### Upgrading from a previous version with E2EE[​](<#upgrading-from-a-previous-version-with-e2ee> "Direct link to Upgrading from a previous version with E2EE")
tip
...
[code] MATRIX_RECOVERY_KEY=EsT... your recovery key here  
...
note
After migration, messages sent _before_ the upgrade cannot be decrypted -- the old encryption keys are gone. This only affects the transition; new messages work normally.
tip
**New installations are not affected.** This migration is only needed if you had a working E2EE setup with a previous version of Hermes and are upgrading.
...
## Proxy Mode (E2EE on macOS)[​](<#proxy-mode-e2ee-on-macos> "Direct link to Proxy Mode \(E2EE on macOS\)")
...
### Step 1: Configure the Host (macOS)[​](<#step-1-configure-the-host-macos> "Direct link to Step 1: Configure the Host \(macOS\)")
...
[/code]
### Step 2: Configure the Docker Container (Linux VM)[​](<#step-2-configure-the-docker-container-linux-vm> "Direct link to Step 2: Configure the Docker Container \(Linux VM\)")
...
### Step 3: Start Both[​](<#step-3-start-both> "Direct link to Step 3: Start Both")
...
### Works for Any Platform[​](<#works-for-any-platform> "Direct link to Works for Any Platform")
...
### Sync issues / bot falls behind[​](<#sync-issues--bot-falls-behind> "Direct link to Sync issues / bot falls behind")
...
### Bot is offline[​](<#bot-is-offline> "Direct link to Bot is offline")
...
### "User not allowed" / Bot ignores you[​](<#user-not-allowed--bot-ignores-you> "Direct link to "User not allowed" / Bot ignores you")
...
## Notes[​](<#notes> "Direct link to Notes")
  ...
  * [How Hermes Behaves](<#how-hermes-behaves>)
    * [Session Model in Matrix](<#session-model-in-matrix>)
    * [Mention and Threading Configuration](<#mention-and-threading-configuration>)
  * [Step 1: Create a Bot Account](<#step-1-create-a-bot-account>)
    * [Option A: Register on Your Homeserver (Recommended)](<#option-a-register-on-your-homeserver-recommended>)
    * [Option B: Use matrix.org or Another Public Homeserver](<#option-b-use-matrixorg-or-another-public-homeserver>)
    * [Option C: Use Your Own Account](<#option-c-use-your-own-account>)
  * [Step 2: Get an Access Token](<#step-2-get-an-access-token>)
    * [Option A: Access Token (Recommended)](<#option-a-access-token-recommended>)
    * [Option B: Password Login](<#option-b-password-login>)
  ...
  * [End-to-End Encryption (E2EE)](<#end-to-end-encryption-e2ee>)
    ...
    * [Enable E2EE](<#enable-e2ee>)
    * [Cross-Signing Verification (Recommended)](<#cross-signing-verification-recommended>)
  * [Home Room](<#home-room>)
  ...
  * [Troubleshooting](<#troubleshooting>)
    ...
    * ["Failed to authenticate" / "whoami failed" on startup](<#failed-to-authenticate--whoami-failed-on-startup>)
    * ["mautrix not installed" error](<#mautrix-not-installed-error>)
    * [Encryption errors / "could not decrypt event"](<#encryption-errors--could-not-decrypt-event>)
    * [Upgrading from a previous version with E2EE](<#upgrading-from-a-previous-version-with-e2ee>)
  * [Proxy Mode (E2EE on macOS)](<#proxy-mode-e2ee-on-macos>)
    * [How It Works](<#how-it-works>)
    * [Step 1: Configure the Host (macOS)](<#step-1-configure-the-host-macos>)
    * [Step 2: Configure the Docker Container (Linux VM)](<#step-2-configure-the-docker-container-linux-vm>)
    * [Step 3: Start Both](<#step-3-start-both>)
    * [Configuration Reference](<#configuration-reference>)
    * [Works for Any Platform](<#works-for-any-platform>)
    * [Sync issues / bot falls behind](<#sync-issues--bot-falls-behind>)
