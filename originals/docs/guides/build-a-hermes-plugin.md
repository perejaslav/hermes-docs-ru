...
## Step 1: Create the plugin directory[​](<#step-1-create-the-plugin-directory> "Direct link to Step 1: Create the plugin directory")
[code] 
    mkdir -p ~/.hermes/plugins/calculator  
    cd ~/.hermes/plugins/calculator  
    
[/code]
## Step 2: Write the manifest[​](<#step-2-write-the-manifest> "Direct link to Step 2: Write the manifest")
Create `plugin.yaml`:
[code] 
    name: calculator  
    version: 1.0.0  
...
[/code]
## Step 3: Write the tool schemas[​](<#step-3-write-the-tool-schemas> "Direct link to Step 3: Write the tool schemas")
...
## Step 4: Write the tool handlers[​](<#step-4-write-the-tool-handlers> "Direct link to Step 4: Write the tool handlers")
...
## Step 5: Write the registration[​](<#step-5-write-the-registration> "Direct link to Step 5: Write the registration")
...
[/code]
## Your plugin's final structure[​](<#your-plugins-final-structure> "Direct link to Your plugin's final structure")
[code] 
...
## What else can plugins do?[​](<#what-else-can-plugins-do> "Direct link to What else can plugins do?")
### Ship data files[​](<#ship-data-files> "Direct link to Ship data files")
...
[/code]
### Bundle skills[​](<#bundle-skills> "Direct link to Bundle skills")
...
### Gate on environment variables[​](<#gate-on-environment-variables> "Direct link to Gate on environment variables")
...
### Conditional tool availability[​](<#conditional-tool-availability> "Direct link to Conditional tool availability")
...
[/code]
### Register multiple hooks[​](<#register-multiple-hooks> "Direct link to Register multiple hooks")
[code] 
    def register(ctx):  
...
### `pre_llm_call` context injection[​](<#pre_llm_call-context-injection> "Direct link to pre_llm_call-context-injection")
...
#### How injection works[​](<#how-injection-works> "Direct link to How injection works")
...
#### Example: Memory recall plugin[​](<#example-memory-recall-plugin> "Direct link to Example: Memory recall plugin")
[code] 
...
[/code]
#### Example: Guardrails plugin[​](<#example-guardrails-plugin> "Direct link to Example: Guardrails plugin")
[code] 
...
[/code]
#### Example: Observer-only hook (no injection)[​](<#example-observer-only-hook-no-injection> "Direct link to Example: Observer-only hook \(no injection\)")
[code] 
...
[/code]
#### Multiple plugins returning context[​](<#multiple-plugins-returning-context> "Direct link to Multiple plugins returning context")
...
### Register CLI commands[​](<#register-cli-commands> "Direct link to Register CLI commands")
...
### Register slash commands[​](<#register-slash-commands> "Direct link to Register slash commands")
...
[/code]
### Dispatch tools from slash commands[​](<#dispatch-tools-from-slash-commands> "Direct link to Dispatch tools from slash commands")
...
## Specialized plugin types[​](<#specialized-plugin-types> "Direct link to Specialized plugin types")
...
### Model provider plugins — add an LLM backend[​](<#model-provider-plugins--add-an-llm-backend> "Direct link to Model provider plugins — add an LLM backend")
...
### Platform plugins — add a gateway channel[​](<#platform-plugins--add-a-gateway-channel> "Direct link to Platform plugins — add a gateway channel")
...
### Memory provider plugins — add a cross-session knowledge backend[​](<#memory-provider-plugins--add-a-cross-session-knowledge-backend> "Direct link to Memory provider plugins — add a cross-session knowledge backend")
...
### Context engine plugins — replace the context compressor[​](<#context-engine-plugins--replace-the-context-compressor> "Direct link to Context engine plugins — replace the context compressor")
[code] 
...
### Image-generation backends[​](<#image-generation-backends> "Direct link to Image-generation backends")
...
## Non-Python extension surfaces[​](<#non-python-extension-surfaces> "Direct link to Non-Python extension surfaces")
...
### MCP servers — register external tools[​](<#mcp-servers--register-external-tools> "Direct link to MCP servers — register external tools")
...
### Gateway event hooks — fire on lifecycle events[​](<#gateway-event-hooks--fire-on-lifecycle-events> "Direct link to Gateway event hooks — fire on lifecycle events")
...
### Shell hooks — run a shell command on tool calls[​](<#shell-hooks--run-a-shell-command-on-tool-calls> "Direct link to Shell hooks — run a shell command on tool calls")
...
### Skill sources — add a custom skill registry[​](<#skill-sources--add-a-custom-skill-registry> "Direct link to Skill sources — add a custom skill registry")
...
### TTS / STT via command templates[​](<#tts--stt-via-command-templates> "Direct link to TTS / STT via command templates")
...
## Distribute via pip[​](<#distribute-via-pip> "Direct link to Distribute via pip")
...
[/code]
## Distribute for NixOS[​](<#distribute-for-nixos> "Direct link to Distribute for NixOS")
...
[/code]
  ...
  * [Step 2: Write the manifest](<#step-2-write-the-manifest>)
  * [Step 3: Write the tool schemas](<#step-3-write-the-tool-schemas>)
  * [Step 4: Write the tool handlers](<#step-4-write-the-tool-handlers>)
  * [Step 5: Write the registration](<#step-5-write-the-registration>)
  * [Step 6: Test it](<#step-6-test-it>)
  * [Your plugin's final structure](<#your-plugins-final-structure>)
  * [What else can plugins do?](<#what-else-can-plugins-do>)
    * [Ship data files](<#ship-data-files>)
    ...
    * [`pre_llm_call` context injection](<#pre_llm_call-context-injection>)
    * [Register CLI commands](<#register-cli-commands>)
    * [Register slash commands](<#register-slash-commands>)
    * [Dispatch tools from slash commands](<#dispatch-tools-from-slash-commands>)
  * [Specialized plugin types](<#specialized-plugin-types>)
    * [Model provider plugins — add an LLM backend](<#model-provider-plugins--add-an-llm-backend>)
    * [Platform plugins — add a gateway channel](<#platform-plugins--add-a-gateway-channel>)
    * [Memory provider plugins — add a cross-session knowledge backend](<#memory-provider-plugins--add-a-cross-session-knowledge-backend>)
    * [Context engine plugins — replace the context compressor](<#context-engine-plugins--replace-the-context-compressor>)
    * [Image-generation backends](<#image-generation-backends>)
  * [Non-Python extension surfaces](<#non-python-extension-surfaces>)
    * [MCP servers — register external tools](<#mcp-servers--register-external-tools>)
    * [Gateway event hooks — fire on lifecycle events](<#gateway-event-hooks--fire-on-lifecycle-events>)
    * [Shell hooks — run a shell command on tool calls](<#shell-hooks--run-a-shell-command-on-tool-calls>)
    * [Skill sources — add a custom skill registry](<#skill-sources--add-a-custom-skill-registry>)
    * [TTS / STT via command templates](<#tts--stt-via-command-templates>)
  * [Distribute via pip](<#distribute-via-pip>)
  * [Distribute for NixOS](<#distribute-for-nixos>)
